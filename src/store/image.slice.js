import { createSlice } from '@reduxjs/toolkit'
import { api } from 'api/index'
import { getLoggedInUserInfo } from 'utils/index';
import { fetchImagesByUserId } from "store/images.slice";

const imageSlice = createSlice({
    name: 'image',
    initialState: {
        image: {},
        isImageSaving: false,
        error: false
    },
    reducers: {
        startSavingImage: state => {
            state.isImageSaving = true;
        },
        hasImageSavingError: (state, action) => {
            state.error = action.payload;
            state.isImageSaving = false;
        },
        imageSavedSuccessfully: (state, action) => {
            state.image = action.payload;
            state.isImageSaving = false;
        },
    },
});

export default imageSlice.reducer;

const { startSavingImage, imageSavedSuccessfully, hasImageSavingError } = imageSlice.actions;

const getCurrentUserId = () => {
    const currentUser = getLoggedInUserInfo();
    if(currentUser) return currentUser.userId;
    return -1;
}
const saveImagePostUrl = `/pin/users/${getCurrentUserId()}/images`;
// Base64 string data
const placeholderReactImageData = '';

export const saveImage = (data) => async dispatch => {
    console.info('image form data: ', data);
    dispatch(startSavingImage());
    /* ==================================================== */
    try{
        await api.get(data.imageURL)
            .then(response=>{
                console.info('image fetch success');
            })
    } catch(error){
        console.error('broken image fetch error')
        // data.isOriginalImageBroken = true;
        // data.brokenImageURL = data.imageURL;
        data.imageURL = `data:image/jpeg;base64,${placeholderReactImageData}`;
    }
    /* ==================================================== */
    
    try {
       await api
        .post(saveImagePostUrl, JSON.stringify(data))
        .then((response) => {
            console.info('Server post response: ', response.data);
            dispatch(imageSavedSuccessfully(response.data));
            if(getCurrentUserId() !== -1){
                dispatch(fetchImagesByUserId(getCurrentUserId()))
            }
            
        })
    }
    catch (e) {
        // return console.error(e.message);
        console.error('ImageSavingError: ', e.message);
        dispatch(hasImageSavingError(e.message))
    }
}